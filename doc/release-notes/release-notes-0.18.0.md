future Core version 0.18.0 is now available from:

  <https://futurecore.org/bin/future-core-0.18.0/>
...
`/Applications/future-Qt` (on Mac) or `futured`/`future-qt` (on
Linux).
...
recommended to use future Core on unsupported systems.
...
10.10. Additionally, future Core does not yet change appearance when
...
`contrib/init/futured.service`, it has been changed to use
`/var/lib/futured` as the data directory instead of
`~future/.future`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/futured` will
exist has enough space (check using `df -h /var/lib/futured`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/future/future/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://futurecore.org/en/contact/) the future Core
...
The systemd init file (`contrib/init/futured.service`) has been changed
to use `/var/lib/futured` as the data directory instead of
`~future/.future`. This change makes future Core more consistent with
...
`datadir` option in `/etc/future/future.conf` will have no effect.
...
- A new short [document](https://github.com/future/future/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/future/future/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/future/future/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/future/future/blob/master/doc/future-conf.md)
  about the `future.conf` file describes how to use it to configure
  future Core.

- A new document introduces future Core's BIP174 [Partially-Signed
  future Transactions
  (PSBT)](https://github.com/future/future/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/future/future/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent future-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  future Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/future/future/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/future/future/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when future Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of future Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/future/future/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/future/future/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart futured with the `-deprecatedrpc=generate`
...
- A new `future-wallet` tool is now distributed alongside future
...
This section describes planned changes to future Core that may affect
other future software and services.

- Since version 0.16.0, future Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with future Core 0.20 (expected about
  a year after 0.18), future Core will default to native segwit
  ...
  to bech32 addresses, and if the future Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in future Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any future Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, future Core
...
- A sub-project of future Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with future Core.  See their [project
  page](https://github.com/future-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to future Core's own implementation, although entropy
  gathered by future Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves future Core a
...
- On macOS, future Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `future_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libfuture_server.a` (jnewbery)
...
- #13926 [Tools] future-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like futured (MarcoFalke)
...
- #14521 qt, docs: Fix `future-qt -version` output formatting (hebasto)
...
- #11625 Add futureApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libfuture_server` (laanwj)
...
- #15154 Configure: future-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/future-wallet (Sjors)
...
- #11526 Visual Studio build configuration for future Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libfuture_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in future-util-test.py (isghe)
...
- #12775 Integration of property based testing into future Core (Christewart)
...
- #12246 Bugfix: Only run future-tx tests when future-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_future` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_future` (MarcoFalke)
...
- #14186 future-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_future.conf` message on startup if conf file exists (leishman)
...
- #12255 Update future.service to conform to init.md (dongcarl)
...
- #14497 Add doc/future-conf.md (hebasto)
...
- #15354 Add missing `future-wallet` tool manpages (MarcoFalke)
...
- #15765 correct futureconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support future folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/future/).
