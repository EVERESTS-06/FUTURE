...
future Core version 0.20.0 is now available from:

  <https://futurecore.org/bin/future-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from future Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since future Core version 0.18.0.
...
- Testing or debugging of implementations of the future P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of future Core. future Core logs debug messages
...
- OpenSSL is no longer used by future Core.  (#17265)

- BIP70 support has been fully removed from future Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/future/future/issues/16599),
  [PR #16702](https://github.com/future/future/pull/16702), and the
  `futured help` for more information.  This option is experimental and
...
- All custom settings configured when future Core starts are now
...
- The "Start future Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://futurecore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple future Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed future Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/future-core/HWI) for
  signing.  Future versions of future Core should support a GUI option
...
- future Core's automatically-generated source code documentation is
  now available at https://doxygen.futurecore.org.  (#17596)
...
- The `future-cli` utility used with the `-getinfo` parameter now
...
- The `future-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `futurestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_future-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBfuture_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_future` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBfuture_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for future-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate future schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest future.conf (sdaftuar)
...
- #18022 Fix appveyor `test_future` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (future-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone future-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about future Core files (hebasto)
...
- #17119 Fix broken future-cli examples (andrewtoth)
...
- #17503 Remove bitness from future-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.futurecore.org (laanwj)
...
- #17648 Rename wallet-tool references to future-wallet (hel-o)
...
- #16115 On futured startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from future-cli -getinfo (malevolent)
