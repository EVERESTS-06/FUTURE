...
Below are some notes on how to build future Core for Windows.

The options known to work for building future Core on Windows are:
...
Note that for WSL the future Core source path MUST be somewhere in the default mount file system, for
example /usr/src/future, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\future`, for example:

    cmake --install build --prefix /mnt/c/workspace/future
