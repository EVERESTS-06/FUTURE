UNIX BUILD NOTES
====================
Some notes on how to build future Core in Unix.
...
memory available when compiling future Core. On systems with less, gcc can be
...
To build future Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/future/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/future/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, future Core can
