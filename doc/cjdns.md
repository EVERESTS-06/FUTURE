# CJDNS support in future Core

It is possible to run future Core over CJDNS, an encrypted IPv6 network that
...
done outside of future Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the future network and individual nodes.
...
before it will work with your future Core node. This is described in steps
...
## Run future Core with CJDNS

Once you are connected to the CJDNS network, the following future Core
...
When enabled, this option tells future Core that it is running in an
...
IPv6 local network. This helps future Core perform better address management:
...
CJDNS support was added to future Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `future-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/future/future/blob/master/doc/tor.md) for
details.

## CJDNS-related information in future Core

There are several ways to see your CJDNS address in future Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `future-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `future-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `future-cli help getnodeaddresses` for details.
