...
Tracing (USDT) support for future Core on Linux using. For more information on
USDT support in future Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `futured` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `futured` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[future Core USDT documentation] on how to list available tracepoints in your
`futured` binary.

[future Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof futured)
```
...
future P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof futured)
```
...
In a different terminal, starting future Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/src/futured -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof futured)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof futured)
```
