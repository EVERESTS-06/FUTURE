# Bootstrappable future Core Builds

This directory contains the files necessary to perform bootstrappable future
Core builds.
...
To build future Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      future Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/future-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/future-core/guix.sigs).
