#!/usr/bin/env python3
# Copyright (c) 2020-2022 The future Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing future-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='future-cli', help='future-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='future address to which the faucet should send')
...
parser.add_argument('future_cli_args', nargs='*', help='Arguments to pass on to future-cli (default: -signet)')

args = parser.parse_args()

if args.future_cli_args == []:
    args.future_cli_args = ['-signet']


def future_cli(rpc_command_and_params):
    argv = [args.cmd] + args.future_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = future_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = future_cli(['getnewaddress', 'faucet', 'bech32'])
