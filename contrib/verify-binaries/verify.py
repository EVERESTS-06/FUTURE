...
"""Script for verifying future Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from futurecore.org and future.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/future-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://futurecore.org"
HOST2 = "https://future.org"
VERSIONPREFIX = "future-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the future community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"future_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by futurecore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the future release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes future.org lags behind futurecore.org.)')
    )
