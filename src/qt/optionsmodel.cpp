...
/** Call node.updateRwSetting() with future 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because future 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/future/future/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because future 22.x will not try to read these.
...
//! Convert enabled/size values to future -prune setting.
...
//! Get pruning enabled value to show in GUI from future -prune setting.
...
//! Get pruning size value to show in GUI from future -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayfutureUnit")) {
        settings.setValue("DisplayfutureUnit", QVariant::fromValue(futureUnit::BTC));
    }
    QVariant unit = settings.value("DisplayfutureUnit");
    if (unit.canConvert<futureUnit>()) {
        m_display_future_unit = unit.value<futureUnit>();
    } else {
        m_display_future_unit = futureUnit::BTC;
        settings.setValue("DisplayfutureUnit", QVariant::fromValue(m_display_future_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if future.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_future_unit);
...
{
    if (new_unit.isNull() || new_unit.value<futureUnit>() == m_display_future_unit) return;
    m_display_future_unit = new_unit.value<futureUnit>();
    QSettings settings;
    settings.setValue("DisplayfutureUnit", QVariant::fromValue(m_display_future_unit));
    Q_EMIT displayUnitChanged(m_display_future_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/future/future/pull/8273
    ...
    // (https://github.com/future-core/gui/issues/567).
