...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<futureUnit>("futureUnit");
#else
    qRegisterMetaType<futureUnit>("futureUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. future_de.qm (shortcut "de" needs to be defined in future.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. future_de_DE.qm (shortcut "de_DE" needs to be defined in future.qrc)
...
static const char* qt_argv = "future-qt";
...
void futureApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the futureApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", futureGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void futureApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool futureApplication::createOptionsModel(bool resetSettings)
{
...
void futureApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new futureGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &futureGUI::quitRequested, this, &futureApplication::requestShutdown);
...
void futureApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void futureApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool futureApplication::baseInitialize()
{
    return node().baseInitialize();
}

void futureApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &futureApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &futureApplication::handleRunawayException);
    connect(this, &futureApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &futureApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void futureApplication::parameterSetup()
{
...
void futureApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void futureApplication::requestInitialize()
{
...
void futureApplication::requestShutdown()
{
...
void futureApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // future: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &futureGUI::handlePaymentRequest);
            connect(window, &futureGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void futureApplication::handleRunawayException(const QString &message)
{
...
void futureApplication::handleNonFatalException(const QString& message)
{
...
WId futureApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool futureApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", futureGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(future);
    Q_INIT_RESOURCE(future_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(future_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see future-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see future-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse future.conf, determine network, switch to network specific
    ...
    // future: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
