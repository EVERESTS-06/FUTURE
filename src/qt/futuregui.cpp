...
const std::string futureGUI::DEFAULT_UIPLATFORM =
...
futureGUI::futureGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &futureGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &futureGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &futureGUI::showModalOverlay);
...
void futureGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a future address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and future: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &futureGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &futureGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &futureGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your future addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified future addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed future Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed future Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a future: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible future command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &futureGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &futureGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &futureGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &futureGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &futureGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &futureGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &futureGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &futureGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &futureGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &futureGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &futureGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &futureGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &futureGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &futureGUI::showDebugWindow);
}

void futureGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void futureGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &futureGUI::setCurrentWalletBySelectorIndex);
...
void futureGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &futureGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &futureGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &futureGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &futureGUI::showProgress);
...
#ifdef ENABLE_WALLET
void futureGUI::enableHistoryAction(bool privacy)
{
...
void futureGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &futureGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &futureGUI::removeWallet);
...
WalletController* futureGUI::getWalletController()
{
    return m_wallet_controller;
}

void futureGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &futureGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &futureGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &futureGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &futureGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &futureGUI::incomingTransaction);
    connect(this, &futureGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void futureGUI::removeWallet(WalletModel* walletModel)
{
...
void futureGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void futureGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void futureGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void futureGUI::setWalletActionsEnabled(bool enabled)
{
...
void futureGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &futureGUI::toggleHidden);
...
void futureGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void futureGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void futureGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void futureGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void futureGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void futureGUI::openClicked()
{
...
void futureGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void futureGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void futureGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void futureGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void futureGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void futureGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void futureGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void futureGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to future network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void futureGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void futureGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void futureGUI::updateHeadersSyncProgressLabel()
{
...
void futureGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void futureGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &futureGUI::quitRequested);
...
void futureGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void futureGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &futureGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void futureGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void futureGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &futureGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &futureGUI::show);
                e->ignore();
            }
        }
    }
...
void futureGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void futureGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void futureGUI::incomingTransaction(const QString& date, futureUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(futureUnits::formatWithUnit(unit, amount, true));
...
void futureGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void futureGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool futureGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool futureGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void futureGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void futureGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void futureGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void futureGUI::updateProxyIcon()
{
    std::string ip_port;
...
void futureGUI::updateWindowTitle()
{
...
void futureGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void futureGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void futureGUI::detectShutdown()
{
...
void futureGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void futureGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(futureGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = futureGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void futureGUI::subscribeToCoreSignals()
{
...
void futureGUI::unsubscribeFromCoreSignals()
{
...
bool futureGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<futureUnit> units = futureUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const futureUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, futureUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const futureUnit u : futureUnits::availableUnits()) {
        menu->addAction(futureUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(futureUnit newUnits)
{
    setText(futureUnits::longName(newUnits));
}
