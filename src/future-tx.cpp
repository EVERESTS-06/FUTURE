...
static void SetupfutureTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " future-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The future-tx tool is used for creating and modifying future transactions.\n\n"
                "future-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded future transaction, or with \"-create [commands]\" to create a hex-encoded future transaction.\n"
                "\n"
                "Usage: future-tx [options] <hex-tx> [commands]\n"
                "or:    future-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded future transaction
