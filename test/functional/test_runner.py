#!/usr/bin/env python3
# Copyright (c) 2014-2022 The future Core developers
...
`test/functional/test_framework/test_framework.py:futureTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_future_cli.py --legacy-wallet',
    'interface_future_cli.py --descriptors',
...
def main():
    ...
    enable_futured = config["components"].getboolean("ENABLE_futureD")

    if not enable_futured:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if futured is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "futured"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a futured process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the future functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/future/future/pull/27561.
...
class RPCCoverage():
    ...
    commands per `future-cli help` (`rpc_interface.txt`).
