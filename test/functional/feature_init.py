...
from test_framework.test_framework import futureTestFramework, SkipTest
...
class InitTest(futureTestFramework):
    """
    ...
    def init_pid_test(self):
        future_PID_FILENAME_CUSTOM = "my_fancy_future_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = future_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / future_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / future_PID_FILENAME_CUSTOM
